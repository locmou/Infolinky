...
void LectureSmartG() {
  ...
  if (!clientESP_RMS.connect(host.c_str(), 82)) {  // PORT 82 pour Smlart Gateways
    Serial.println("connection to client SmartGateways failed (call from LectureSmartG)");
    delay(200);
    WIFIbug++;
    return;
  }
  ...
  while (clientESP_RMS.available() == 0) {
    if (millis() - timeout > 5000) {
      Serial.println(">>> client SmartGateways Timeout !");
      clientESP_RMS.stop();
      return;
    }
  }
  ...
  PuissanceS_M=PintMax(int(ValJsonSG("PowerDelivered_total", SmartG_Data)));
  PuissanceI_M=PintMax(int(ValJsonSG("PowerReturned_total", SmartG_Data)));
