  ...
  if ((LastH != Hcour) && (Hcour == 0 || Hcour == 1 || Hcour == 10 || Hcour == 50 || Hcour == 300 || Hcour == 530 || Hcour == 560 || Hcour == 600 || Hcour == 800 || Hcour == 1000) || LastHeureEDF < 0) {
    if (TempoEDFon == 1) {
      ...
      if (!clientSecuEDF.connect(adr_EDF_Host, 443)) {
        Serial.println("Connection failed to EDF server :" + Host);
      } else {
        ...
        while (clientSecuEDF.available() == 0) {
          if (millis() - timeout > 5000) {
            Serial.println(">>> clientSecuEDF EDF Timeout !");
            clientSecuEDF.stop();
            return;
          }
        }
        ...
        while (clientSecuEDF.connected() && (millis() - timeout < 5000) && fin < 2) {
          ...
          EDFdata += line;
          if (line == "\r") {
            Serial.println("EnTetes EDF reçues");
            EDFdata = "";
            fin = 1;
          }
        ...
        if (LTARFrecu.indexOf("TEMPO") >= 0) {
          ...
          Serial.println(DateEDF + " : " + EDFdata);
          EDFdata = "";
          LastHeureEDF = HeureCouranteDeci;  //Heure lecture Tempo EDF
        } else {
          Serial.println(DateEDF + " : Pas de données EDF valides");
        }
      }
    } else {
