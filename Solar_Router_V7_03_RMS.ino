/*
  ...
  ****************************************
  ...
  - Lecture depuis un autre ESP qui comprend l'une des 4 sources citée plus haut
  ...
  - V7.01_RMS Corrige un bug sur l'affichage de l'energie quotidienne dans le cas ESP externe
...
*/
#define Version "7.03_RMS"
#define HOSTNAME "RMS-ESP32-"
#define CLE_Rom_Init 702567807  //Valeur pour tester si ROM vierge ou pas. Un changement de valeur remet à zéro toutes les données. / Value to test whether blank ROM or not.
...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("Can not connect to WiFi station. Go into AP mode and STA mode.");
...
void GestionOverproduction() {
  ...
  if (LesActions[0].Actif == 1 && LesActions[0].TypeEnCoursTriac(HeureCouranteDeci, temperature, LTARFbin) >= 2) {
    ...
    retardF = retardF + 0.001;                                           //On ferme très légèrement si pas de message reçu
    retardF = retardF + (Puissance - TriacSeuilPw) * GainBoucle / 4000;  // Gain de boucle de l'asservissement
  ...
  if (retard == 100) {              // Force en cas d'arret des IT
    digitalWrite(pulseTriac, LOW);  //Stop Triac
  }
  //Cas des GPIOS
  for (int i = 1; i < NbActions; i++) {
    LesActions[i].Activer(Puissance, HeureCouranteDeci, temperature, LTARFbin);
  }
}
